...
+ 创建MySQL修改日志表`modify_logs`

```mysql
USE bank_db;

DROP TABLE IF EXISTS modify_logs;
CREATE TABLE modify_logs (
    LogID INT AUTO_INCREMENT PRIMARY KEY, -- 日志记录的唯一标识符
    TimeStamp DATETIME NOT NULL,          -- 修改时间戳
    Action ENUM('INSERT', 'UPDATE', 'DELETE') NOT NULL, -- 操作类型
    RowId INT,                            -- 被修改的记录 ID
    ModifiedColumn VARCHAR(255),          -- 被修改的列名称
    OldValue TEXT,                        -- 修改前的值
    NewValue TEXT                         -- 修改后的值
);
```

+ 安装python连接MySQL的库和随机生成库

```bash
pip install pymysql faker==3.0.1
```

+ 使用python脚本`generate_log.py`随机生成10秒内的修改日志包括UPDATE/DELETE/INSERT并导入表`modify_logs`
  + 可选是否添加一个参数 , 若不添加则使用实时时间 , 若添加则首先判断是否为`item_by_item`字符串 , 若是则每隔几秒插入一条数据(为了方便实时化演示) , 若不是则判断是否为日期 , 生成对应日期的按具体时间排序的20条日志 . 
  + 生成的NewValue为Json格式 , 若是INSERT/UPDATE操作则其值为插入后/修改后的新json格式字符串 , 若是DELETE操作则其值为NULL . 
